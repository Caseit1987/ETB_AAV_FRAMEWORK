...
"
        ...
        if not nonce_ok:
            ...
            client.close(); return
        log_entry = {"timestamp": datetime.utcnow().isoformat(), "status": "PASS", "nonce": parsed["nonce"], "src_chain": parsed["src_chain"], "payload_hash": hashlib.sha256(parsed["payload"]).hexdigest()[:16]}
...
")
        try:
            ...
            client.sendall(http_ok_header + response)
            backend.close()
        except: pass
...
def run_etb_gate():
    ...
    gateway.listen(128)
    print(f"[+] Stateful Protocol Proxy active on port {BIND_PORT}...")
    while True:
        try:
